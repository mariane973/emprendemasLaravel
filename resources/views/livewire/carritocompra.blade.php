      ...
      @else
        @foreach($carritoitems as $item)
          <div class="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start" style="min-height: 200px;">
            @if($item->producto)
              <img src="imagenes/productos/{{$item->producto->imagen}}" alt="product-image" class="w-40 h-40 object-cover rounded-lg sm:w-40 d-block mx-auto mb-4 xs:mx-auto" />
              <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between sm:items-center">
                <div>
                  <h2 class="text-lg font-bold text-gray-900 text-center">{{$item->producto->nombre}}</h2>
                  <p class="mt-1 text-xs text-gray-700 text-center" style="font-size: 18px">${{number_format($item->producto->valor_final)}}</p>
                </div>
                <div class="mt-4 flex flex-col items-center sm:space-y-6 sm:mt-0 sm:flex sm:flex-row sm:items-center sm:space-x-6">
                  ...
                  <div class="flex items-center space-x-4 mt-2 sm:mt-0">
                    <p class="mb-2 fw-semibold text-center" style="font-size: 18px; color: green">${{number_format($item->producto->valor_final * $item->cantidad)}}</p>
            ...
            @elseif($item->servicio)
              <img src="imagenes/servicios/{{$item->servicio->imagen}}" alt="servicio-image" class="w-40 h-40 object-cover rounded-lg sm:w-40 d-block mx-auto mb-4 xs:mx-auto" />
              <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between sm:items-center">
                <div>
                  <h2 class="text-lg font-bold text-gray-900 text-center">{{$item->servicio->nombre}}</h2>
                  <p class="mt-1 text-xs text-gray-700 text-center" style="font-size: 18px">${{ number_format($item->servicio->valor_final)}}</p>
                </div>
                <div class="mt-4 flex flex-col items-center sm:space-y-6 sm:mt-0 sm:flex sm:flex-row sm:items-center sm:space-x-6">
                  <div class="flex items-center gap-1 border-gray-100">
                    <button class="w-7 h-7 rounded-full border border-gray-300 cursor-pointer" wire:click="decrementCant({{ $item->id }})">
                  ...
                  <div class="flex items-center space-x-4 mt-2 sm:mt-0">
                    <p class="mb-2 fw-semibold text-center" style="font-size: 18px; color: green">${{number_format ($item->servicio->valor_final* $item->cantidad) }}</p>
